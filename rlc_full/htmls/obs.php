...
require_once '../php/gerenciar_representante.php';
session_start();
...
if ($nivel_acesso === 'representante') {
    $representante = verificarUsuarioRepresentante($conexao, $id_usuario);
    if (!$representante) {
        ?>
        <style>
        ...
        // Define a turma que o representante representa
        $id_turma = $representante['id_turma'];
    }
}
...
<body class="obs">
    ...
    <main class="obs-main-container">
        ...
        <form method="post" action="../php/salvar_obs.php<?= $editavel ? '' : '?data=' . urlencode($data_selecionada) ?>" class="obs-table-form">
            <div class="obs-table-wrapper table-responsive">
                <table>
                    <thead>
                        <tr>
                            <th>Nº</th>
                    ...
                    <tbody>
                        ...
                        <tr<?php if ($aluno['status'] == 'transferido') echo ' class="transferido"'; ?>>
                            <td data-label="Nº"><?= htmlspecialchars($aluno['numero_chamada']) ?></td>
                            <td class="nome-col" data-label="Nome"><?= htmlspecialchars($aluno['nome']) ?></td>
